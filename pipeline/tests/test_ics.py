...
def test_uid_is_stable_across_two_builds():
    ...
    game_a = _game("EVT1")
    game_b = _game("EVT1")
    cal_a = build_calendar([game_a], cal_name="Test A", dtstamp=BUILD_TIME)
    cal_b = build_calendar([game_b], cal_name="Test B", dtstamp=BUILD_TIME)
...
def test_different_events_get_different_uids():
    ...
    cal = build_calendar([game1, game2], cal_name="Test", dtstamp=BUILD_TIME)
...
def test_calendar_round_trips_through_icalendar_parser():
    ...
    game = _game("EVT1")
    cal = build_calendar([game], cal_name="Indiana Fever", dtstamp=BUILD_TIME)
...
def test_date_tbd_game_excluded_from_ics_but_not_erased():
    ...
    cal = build_calendar([tbd_game, normal_game], cal_name="Test", dtstamp=BUILD_TIME)
    events = cal.walk("VEVENT")
    assert len(events) == 1
...
def test_no_price_description_says_not_available():
    ...
    cal = build_calendar([game], cal_name="Test", dtstamp=BUILD_TIME)
...
def test_the_link_back_never_touches_a_uid():
    ...
    uids = {
        str(league_calendar("wnba", "WNBA", [game], base_url=base, dtstamp=BUILD_TIME).walk("VEVENT")[0]["uid"])
